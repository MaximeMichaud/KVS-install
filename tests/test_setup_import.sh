...
test_setup_and_init_are_wired_for_imports() {
    ...
    grep -Fq "'^https?://(www[.])?\${HOST_PATTERN}(:[0-9]+)?/contents/'" "$configure" || fail "http storage URLs must be adopted too"
    grep -Fq 'HOST_PATTERN="(${DOMAIN_PATTERN}|${IMPORT_SOURCE_DOMAIN//./[.]})"' "$configure" || fail "storage URLs on the source domain must follow the installation"
    grep -Fq 'IMPORT_SOURCE_DOMAIN=${IMPORT_SOURCE_DOMAIN:-}' "$REPO_ROOT/docker/docker-compose.yml" || fail "the init must receive the source domain"
    grep -Fq 'set_env_value IMPORT_SOURCE_DOMAIN "$IMPORT_DETECTED_DOMAIN"' "$setup" || fail "the source domain must reach .env for the init"
    grep -Fq 'IMPORT_REUSE_SITE_DIR=DIR' "$setup" || fail "the usage must document IMPORT_REUSE_SITE_DIR"
    grep -Fq 'import_take_over_site "$IMPORT_REUSE_SITE_DIR" "/var/www/$DOMAIN"' "$setup" || fail "the files of an earlier import under another domain must be taken over before the transfer"
    grep -Fq 'IMPORT_REUSE_SITE_DIR goes with IMPORT_REMOTE_HOST' "$setup" || fail "the take-over must be refused with the other sources"
    grep -Fq 'import_remote_free_space_check "$IMPORT_REMOTE_REPORT" "/var/www/$DOMAIN" "ssh://' "$setup" || fail "a later pass must check the free space for the dump alone"
    grep -Fq 'the import completed under the old one does not count' "$setup" || fail "a domain change must forget the completion of the earlier import"
    grep -Fq 'Removing the containers of ${PREVIOUS_PROJECT} (${PREVIOUS_DOMAIN}); its volumes stay' "$setup" || fail "an import under a new domain must retire the earlier stack and say what stays"
    grep -B1 -F "sed -i 's/^SITE_PREFIX=.*/SITE_PREFIX=kvs/' .env" "$setup" | grep -Fq 'The prefix is chosen again' || fail "an import under a new domain must choose the prefix again"
