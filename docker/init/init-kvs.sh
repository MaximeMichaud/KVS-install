...
# Manticore Search configuration (if enabled)
if [ "${ENABLE_MANTICORE:-false}" = "true" ]; then
    ...
    echo "Downloading Manticore search scripts..."
    ...
    echo "✓ Manticore PHP scripts configured and installed"

    # Configure External Search plugin automatically
    echo "Configuring External Search plugin..."
...
<?php
$plugin_data = array(
    'enable_external_search' => 1,  // Always enabled
    'display_results' => 1,  // Completely replace internal search
...
EOPHP
    ...
    echo ""
    echo "✓ KVS External Search plugin configured:"
    echo "  - Videos: http://manticore-api/kvs_manticore_search_videos.php"
    echo "  - Albums: http://manticore-api/kvs_manticore_search_albums.php"
    echo "  - Searches: http://manticore-api/kvs_manticore_search_searches.php"
    echo ""
