...
- **Unknown hosts dropped**: a request for the bare IP address or for a name the site does not serve gets no answer and no redirect (a catch-all server in the Docker vhost closes it, and on 443 refuses the TLS handshake before any certificate is sent), so scanning the address gives away neither the site nor its domain.
